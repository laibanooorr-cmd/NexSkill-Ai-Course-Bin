...
print("Counting the rows and columns in DataFrame using shape() : " ,df.shape) # output  (200, 12)
...
# index_col='brokered by'
df_index_col = pd.read_csv('RealEstate-USA.csv',delimiter=",",parse_dates=[11], date_format={'date_added': '%d-%m-%Y'} , index_col='brokered_by')
...
#Second cycle - with index_col as brokered by
#Selecting a single row using .loc
first_row = df_index_col.loc[52707]
print("Selecting a single row using .loc")
print(first_row)

##Selecting multiple rows using .loc
first_row2 = df_index_col.loc[[1205, 19383]]
print("#Selecting multiple rows using .loc")
print(first_row2)


#Selecting a slice of rows using .iloc
first_row3 = df_index_col.loc[19383:62210]
print("Selecting a slice of rows using .loc")
print(first_row3)


# #Conditional selection of rows using .loc
first_row4 = df_index_col.loc[df_index_col['status'] == 'for_sale']
print("Conditional selection of rows using .loc")
print(first_row4)

# #Selecting a single column using .iloc
first_row5 = df_index_col.loc[:1205,'price']
print("Selecting a single column using .iloc")
print(first_row5)

# #Selecting multiple columns using .loc
first_row6 = df_index_col.loc[:1205,['price', 'status']]
print("Selecting multiple columns using .loc")
print(first_row6)

# #Selecting a slice of columns using .loc
first_row7 = df_index_col.loc[:1205,'status':'price']
print("Selecting a slice of columns using .loc")
print(first_row7)

# #Combined row and column selection using .loc
first_row8 = df_index_col.loc[df_index_col['status'] == 'for_sale','street':'city']
print("#Combined row and column selection using .loc")
print(first_row8)
...
df.drop(['bed', 'city'], axis=1, inplace=True)
# display the modified DataFrame after deleting rows
...
df.rename(columns= {'bed': 'bed_changed'}, inplace=True)
# rename columns 'Age' and 'City'
df.rename(mapper= {'bath': 'bath_Changed', 'acre':'acre_Changed'}, axis=1, inplace=True)
...
# select the rows where the house_size is 20000 or bath is 2
selected_rows = df.query('house_size == 2000' or  'bath >= 2')
...
sorted_df = df.sort_values(by='prev_sold_date')
print(sorted_df.to_string(index=False))


#Sort Pandas DataFrame by Multiple Columns
df1 = df.sort_values(by=['prev_sold_date', 'brokered_by'])

print("Sorting by 'prev_sold_date' (ascending) and then by 'brokered_by' (ascending):\n")
print(df1.to_string(index=False))



# calculate the sum of prev_sold_date for each category
grouped = df.groupby('brokered_by')['prev_sold_date'].sum()
