...
df = pd.read_csv('Real_Estate_Sales_2001-2022_GL-Short (1).csv',delimiter=",",parse_dates=[13], date_format={'date_added': '%d-%m-%Y'})
...
print("Counting the rows and columns in DataFrame using shape() : " ,df.shape) # output  (142, 14)

# access the Name column
Price = df['Sale Amount']
print("access the Name column: df : ", Price)

# access multiple columns
price_status = df[['Sale Amount','Assessed Value']]
...
df_index_col = pd.read_csv('Real_Estate_Sales_2001-2022_GL-Short (1).csv',delimiter=",",parse_dates=[13], date_format={'date_added': '%d-%m-%Y'} , index_col='Serial Number')
...
#Selecting a single row using .loc
first_row = df_index_col.loc[2020177]
print("Selecting a single row using .loc")
print(first_row)

##Selecting multiple rows using .loc
first_row2 = df_index_col.loc[[2020177, 20058]]
print("#Selecting multiple rows using .loc")
print(first_row2)

#Selecting a slice of rows using .iloc
first_row3 = df_index_col.loc[2020177:20058]
print("Selecting a slice of rows using .loc")
print(first_row3)


# #Conditional selection of rows using .loc
first_row4 = df_index_col.loc[df_index_col['Town'] == 'Ansonia']
print("Conditional selection of rows using .loc")
print(first_row4)

# #Selecting a single column using .iloc
first_row5 = df_index_col.loc[:2020177 ]
print("Selecting a single column using .iloc")
print(first_row5)

# #Selecting multiple columns using .loc
first_row6 = df_index_col.loc[:2020177,['Town', 'Address']]
print("Selecting multiple columns using .loc")
print(first_row6)

# #Selecting a slice of columns using .loc
first_row7 = df_index_col.loc[:2020177,'Address':'Town']
print("Selecting a slice of columns using .loc")
print(first_row7)

# #Combined row and column selection using .loc
first_row8 = df_index_col.loc[df_index_col['Town'] == 'Ansonia','Address':'Location']
print("#Combined row and column selection using .loc")
print(first_row8)
...
# #Selecting a slice of rows using .iloc
first_row3 = df_index_col.iloc[3:6]
...
df.drop(columns='List Year', inplace=False)
# delete height and profession columns
df.drop(['Location', 'List Year'], axis=1, inplace=True)
# display the modified DataFrame after deleting rows
...
df.rename(mapper= {'Location': 'Location_Changed', 'Address':'Address_Changed'}, axis=1, inplace=True)
...
# select the rows where Town is "Ansonia" or date recorded is "11/02/2020"
selected_rows = df.query(' Town_changed  == \'Ansonia\' or `Date Recorded` == "11/02/2020"')
...
df1 = df.sort_values(by=['Date Recorded', 'Address_Changed'])

print("Sorting by 'Date Recorded' (ascending) and then by 'Address_Changed' (ascending):\n")
print(df1.to_string(index=False))


# calculate the sum of prev_sold_date for each category
grouped = df.groupby('Address_Changed')['Date Recorded'].sum()
